  ...
  async fetch(request, env) {
    ...
    try {
      ...
      try {
        ...
        return Response.json({ error: "无效的 JSON 数据" }, { 
            status: 400,
            headers: { "Access-Control-Allow-Origin": "*" }
        });
      }
      ...
      // 1. 验证输入
      if (!pathname || typeof pathname !== "string" || pathname.length < 1 || pathname.length > 20) {
        return Response.json({ error: "无效的路径名 (5-20 字符)" }, { 
            status: 400,
            headers: { "Access-Control-Allow-Origin": "*" }
        });
      }
      ...
      if (!url || typeof url !== "string" || url.length > 300) {
        return Response.json({ error: "无效的 URL (最大 300 字符)" }, { 
            status: 400,
            headers: { "Access-Control-Allow-Origin": "*" }
        });
      }
      try {
        ...
        if (parsedUrl.protocol !== "http:" && parsedUrl.protocol !== "https:") {
            return Response.json({ error: "无效的 URL 协议 (只允许 http/https)" }, { 
                status: 400,
                headers: { "Access-Control-Allow-Origin": "*" }
            });
        }
        ...
        if (baseDomain) {
            ...
            if (parsedUrl.hostname.toLowerCase() === baseDomain.toLowerCase()) {
                return Response.json({ error: "不能重定向到短链接服务本身 (循环保护)" }, { 
                    status: 400,
                    headers: { "Access-Control-Allow-Origin": "*" }
                });
            }
       ...
       try {
           ...
           if (!isIp) {
               // 使用域名 endpoint，避免 IP 证书验证问题
                ...
                console.log("DoH 请求:", dohUrl);
                
                const dohResp = await fetch(dohUrl, {
                    headers: { "Accept": "application/dns-json" }
                });
                
                console.log("DoH 状态:", dohResp.status);
                
                if (dohResp.ok) {
                   const dnsData = await dohResp.json();
                   console.log("DoH 响应体:", JSON.stringify(dnsData));
                   ...
                   if (dnsData.Answer) {
                       for (const answer of dnsData.Answer) {
                           ...
                           if (answer.data === "0.0.0.0" || answer.data === "::") {
                               const reason = dnsData.Comment ? dnsData.Comment.join(", ") : "恶意软件/成人内容";
      ...
      if (!getResp.ok) {
        ...
        console.error("GitHub 获取错误:", errText);
        return Response.json({ error: "获取 GitHub 文件失败: " + getResp.status }, { 
            status: 502,
            headers: { "Access-Control-Allow-Origin": "*" }
        });
      }
      ...
      const commitMessage = `添加短链: ${pathname}`;
      ...
      if (!putResp.ok) {
        ...
        console.error("GitHub API 错误:", errText);
      ...
      console.error(err);
      return Response.json({ error: "服务器内部错误: " + err.message }, { 
          status: 500,
          headers: { "Access-Control-Allow-Origin": "*" }
      });
    }
